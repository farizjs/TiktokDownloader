...
def start(client, message):
    kb = [[InlineKeyboardButton('Channel 🛡', url=chnnl),InlineKeyboardButton('Repo 🔰', url="https://github.com/farizjs/TikTokDownloader/")]]
    reply_markup = InlineKeyboardMarkup(kb)
    app.send_message(chat_id=message.from_user.id, text=f"Halo, saya **TikTok Downloader**.\nSaya dapat mengunduh video TikTok tanpa Watermark.\n\n"
                          "__**Developer :**__ __[Fariz](t.me/farizsj)__\n"
...
def tiktok_dl(client, message):
    ...
    with requests.get(r, timeout=(50, 10000), stream=True) as r:
                ...
                if show == 1:
                    try:
                        a.edit(f'__**URL :**__ __{message.text}__\n'
                               f'__**Ukuran Total :**__ __{total_size} MB__\n'
        ...
        a.edit(f'__Diunduh ke server!\n'
               f'Mengunggah ke Telegram Sekarang ⏳__')
        start = time.time()
        title = filename
        app.send_document(chat_id=message.chat.id,
                          document=f"./{directory}/{filename}",
                          caption=f"**File :** __{filename}__\n"
                          f"**Ukuran :** __{total_size} MB__\n\n"
                          f"__Diunggah oleh @{BOT_URL}__",
